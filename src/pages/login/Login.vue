<template>
    <div class="flex justify-center items-center bg-slate-900 h-full bg-banner bg-cover">
      <form @submit.prevent="enviarDatos" class="items-center rounded-xl border-2 border-gray-400/10 shadow-lg bg-base-400/60 gap-4 flex flex-col py-6 px-24">
        ...
        <section class="flex flex-col gap-4">
          <article class="flex flex-col m-1 gap-2">
            ...
            <label class="input input-bordered input-primary input-sm bg-primary/10 flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="24" viewBox="0 0 24 24" fill="currentColor" class="icon icon-tabler icons-tabler-filled icon-tabler-user text-gray-300">
          ...
          <article class="flex flex-col m-1 gap-2">
            ...
            <label class="input input-bordered input-primary input-sm bg-primary/10 flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="icon icon-tabler icons-tabler-outline icon-tabler-password text-gray-300">
                <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                <path d="M12 10v4" />
                <path d="M10 13l4 -2" />
                <path d="M10 11l4 2" />
        ...
        <small class="text-xs">¿No tienes cuenta? <RouterLink to="/registro/freelancer" class="link text-secondary">¡Regístrate ahora!</RouterLink></small>
      </form>
    </div>
  </template>
  ...
  try {
    ...
    if (response && response.usuario && response.usuario.id_usuario) {
